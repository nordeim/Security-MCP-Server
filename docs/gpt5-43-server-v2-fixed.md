### Drop-in replacement server.py

This replacement removes brittle, import-time dependency flags and validates HTTP dependencies at runtime with explicit error context. It preserves existing behavior and endpoints while making startup deterministic and easier to debug.
...
try:
    ...
    MCPServerBase = None  # type: ignore
...
if BaseModel:
    ...
    ToolExecutionRequest = None  # type: ignore
...
class EnhancedMCPServer:
    ...
    async def run_stdio_original(self):
        ...
        async with stdio_server() as (read_stream, write_stream):
            await self.server.run(
                read_stream, write_stream, self.shutdown_event
            )
    ...
    def _validate_http_deps(require: bool = True) -> bool:
        ...
        def _try_import(mod: str):
            try:
                ...
                details.append(f"{mod}={getattr(m, '__version__', 'unknown')}")
                return True
            except Exception as e:
        ...
        # Common extras used by FastAPI apps (optional but frequently required)
        # Log missing but don't fail solely on these unless they are used in app construction.
        for mod in ("orjson", "python_multipart", "jinja2", "uvloop", "httptools"):
            if not _try_import(mod):
    ...
    async def run_http_enhanced(self):
        ...
        # Validate deps at runtime using the active interpreter (venv)
        ...
        async def execute_tool(
            tool_name: str,
            request: ToolExecutionRequest,  # type: ignore
            background_tasks: BackgroundTasks  # type: ignore
        ):
            """Execute a tool with validated input."""
            tool = self.tool_registry.get_tool(tool_name)
            if not tool:
                raise HTTPException(status_code=404, detail=f"Tool {tool_name} not found")  # type: ignore

            if tool_name not in self.tool_registry.enabled_tools:
                raise HTTPException(status_code=403, detail=f"Tool {tool_name} is disabled")  # type: ignore
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail="Tool execution failed")  # type: ignore

        @app.get("/events")
        async def events(request: Request):  # type: ignore
            """SSE endpoint for real-time updates."""
            async def event_generator():
                while not await request.is_disconnected():  # type: ignore
        ...
        async def metrics():
            ...
            if metrics_text:
                return Response(content=metrics_text, media_type=CONTENT_TYPE_LATEST)  # type: ignore
            return JSONResponse(content=self.metrics_manager.get_all_stats())  # type: ignore
        ...
        if uvicorn is None:
            raise RuntimeError("Uvicorn not importable at runtime")
...
async def _serve(server: MCPServerBase, shutdown_grace: float) -> None:  # type: ignore
    ...
    serve_task = asyncio.create_task(server.serve(), name="mcp_serve")  # type: ignore
...
async def main_enhanced() -> None:
    ...
    try:
        if transport == "stdio" and server.server:
            await _serve(server.server, shutdown_grace=shutdown_grace)  # type: ignore
        else:
            await server.run()
    finally:
...
- Replaced brittle import-time flags with runtime validation in `_validate_http_deps()`. This ensures FastAPI/Uvicorn are checked using the active interpreter (your venv) and logs missing modules explicitly.
- Kept all endpoints and behavior intact.
- Added informative logs with module versions when available.
- Guarded optional extras (orjson, multipart, jinja2, uvloop, httptools) to aid debugging without hard-failing unless the core stack is missing.
...
- Check logs:
  ...
  - You should see `enhanced_server.http_deps_ok` followed by `enhanced_server.start_http_enhanced`.
- Health:
  - Container should transition to `healthy`.
- Sanity checks:
  - `curl http://localhost:8080/health`
  - `curl http://localhost:8080/metrics`

If anything else surfaces in logs, paste the last 50 lines and I’ll pinpoint the next fix.

https://copilot.microsoft.com/shares/pUwAvrg1tCxuPdej6peg7
