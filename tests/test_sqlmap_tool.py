...
def test_url_and_batch_required(tool: SqlmapTool, make_input, assert_validation_error) -> None:
    inp = make_input(target="http://192.168.0.5/item?id=1")
    output = pytest.run(asyncio=True)(tool._execute_tool(inp, None))  # type: ignore[attr-defined]
...
def test_placeholder_payload_restore(tool: SqlmapTool, make_input) -> None:
    ...
    restored = tool._parse_and_validate_args(secured, make_input(target="http://192.168.0.5/item?id=1"))
...
def test_invalid_payload_token_rejected(tool: SqlmapTool, make_input, assert_validation_error) -> None:
    ...
    secured = tool._secure_sqlmap_args(payload)
    output = tool._parse_and_validate_args(secured, make_input(target="http://192.168.0.5/item?id=1"))
